...
public class AgentController {
    ...
    @Operation(summary = "Tableau de bord agent",
            description = "Obtenir toutes les informations du dashboard agent (check-in, check-out, alertes, etc.)")
    ...
    @Operation(summary = "File d'attente check-out",
            description = "Obtenir les check-out en attente")
    @PreAuthorize("hasRole('AGENT') or hasRole('ADMIN') or hasRole('MANAGER')")
    ...
    @Operation(summary = "File d'attente check-in",
            description = "Obtenir les check-in en attente")
    @PreAuthorize("hasRole('AGENT') or hasRole('ADMIN') or hasRole('MANAGER')")
     ...
     */
    @GetMapping("/alerts")
    @Operation(summary = "Alertes agent",
            description = "Obtenir les alertes (retards, check-in manqués, etc.)")
    @PreAuthorize("hasRole('AGENT') or hasRole('ADMIN') or hasRole('MANAGER')")
    ...
    @Operation(summary = "Réservations du jour",
            description = "Obtenir toutes les réservations prévues aujourd'hui")
    @PreAuthorize("hasRole('AGENT') or hasRole('ADMIN') or hasRole('MANAGER')")
    ...
    @Operation(summary = "Transactions complétées",
            description = "Obtenir les check-in/out complétés aujourd'hui")
    @PreAuthorize("hasRole('AGENT') or hasRole('ADMIN') or hasRole('MANAGER')")
