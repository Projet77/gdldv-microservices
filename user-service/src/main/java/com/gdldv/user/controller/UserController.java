...
public class UserController {
    ...
    /**
     * Récupérer un utilisateur par ID (Admin, Manager, Employee uniquement)
     * GET /api/users/{id}
     */
    @GetMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN') or hasRole('MANAGER') or hasRole('EMPLOYEE') or hasRole('SUPER_ADMIN')")
    @Operation(summary = "Récupérer un utilisateur par ID", description = "Récupérer les informations d'un utilisateur spécifique (Admin/Manager/Employé)")
    ...
    /**
     * Récupérer tous les utilisateurs (Admin, Manager uniquement)
     * GET /api/users
     */
    @GetMapping
    @PreAuthorize("hasRole('ADMIN') or hasRole('MANAGER') or hasRole('SUPER_ADMIN')")
    @Operation(summary = "Récupérer tous les utilisateurs", description = "Liste de tous les utilisateurs (Admin/Manager uniquement)")
    ...
    public ResponseEntity<?> updateUser(@PathVariable Long id,
            @Valid @RequestBody com.gdldv.user.dto.UpdateUserRequest request) {
        try {
    ...
    public ResponseEntity<?> uploadIdentityCard(
    ...
    public ResponseEntity<?> uploadPassport(
            @RequestParam("file") org.springframework.web.multipart.MultipartFile file) {
        try {
    ...
    public ResponseEntity<?> uploadDriverLicense(
            @RequestParam("file") org.springframework.web.multipart.MultipartFile file) {
        try {
